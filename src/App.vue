<template>
  <div id="app">
    ...
    <main id="page-wrap">
      ...
      <h2 class="description">An off-canvas sidebar vue component with a collection of effects and styles using CSS
        transitions and SVG path animations.</h2>

      <nav class="demo-buttons">
        <span v-for="(menu, index) in menus" :key="index">
          <span>
            <a :class="currentMenu === menu.buttonText.replace(/ +/g, '').toLowerCase() ? {currentDemo:true}: {currentDemo:false}"
              @click="changeMenu(menu.buttonText)">
              {{menu.buttonText}}
            </a>
          </span>
        </span>
